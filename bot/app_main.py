...
from bot.config import AppConfig, AssetConfig, ResearchCapitalConfig, load_config
    ...
    *,
    ...
    for item in capitals:
        ...
        if "USDPLN" not in fx_static_rates:
            raise RuntimeError(
                "Optimizer capital currency PLN requires fx_static_rates.USDPLN for conversion model."
            )
...
def _apply_cli_overrides(args: argparse.Namespace, config: AppConfig) -> None:
    ...
    if args.research_capitals is not None:
        ...
        config.research.optimize.capitals = [
            ResearchCapitalConfig(equity=float(item["equity"]), currency=str(item["currency"]))
            for item in parsed
        ]
...
def _extract_source_reports_from_report_dir(report_dir: Path) -> list[dict[str, object]]:
    ...
    for report_path in sorted(report_dir.rglob("report.json")):
        ...
        source_report.setdefault("profit_factor_net", metrics.get("profit_factor_net", metrics.get("profit_factor", 0.0)))
...
def run_research_optimize_mode(args: argparse.Namespace, config: AppConfig, assets: list[AssetConfig], root: Path) -> None:
    ...
    capital_run_mode = str(
        args.research_capital_run_mode if args.research_capital_run_mode is not None else optimize_cfg.capital_run_mode
    ).strip().lower()
    ...
    def _run_single_capital(capital_spec: dict[str, object]) -> dict[str, object]:
        ...
        checkpoint_meta["completed"] = False
        save_checkpoint(checkpoint_path, checkpoint)
        (capital_dir / "search_space.json").write_text(json.dumps(search_space_out, indent=2, ensure_ascii=True), encoding="utf-8")
        (capital_dir / "split_info.json").write_text(json.dumps(split_payload, indent=2, ensure_ascii=True), encoding="utf-8")
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for candidate in stage_a_candidates:
                ...
                candidate_config_path = candidate_dir / "candidate_config.yaml"
                candidate_config_path.write_text(yaml.safe_dump(candidate_config_payload, sort_keys=False, allow_unicode=False), encoding="utf-8")
                future = executor.submit(
                    _run_optimizer_window_candidate,
                    root=root,
                    args=args,
        ...
        top_gate_candidates = [{"candidate_id": item.get("candidate_id"), "gate_mode": item.get("gate_mode"), "gate_params": item.get("gate_params", {})} for item in stage_a_results[:top_gate_keep]]
        stage_b_candidates = build_stage_b_candidates(top_gate_candidates=top_gate_candidates, risk_profiles=risk_profiles, runtime_budget=runtime_budget)
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for candidate in stage_b_candidates:
                ...
                candidate_config_path = candidate_dir / "candidate_config.yaml"
                candidate_config_path.write_text(yaml.safe_dump(candidate_config_payload, sort_keys=False, allow_unicode=False), encoding="utf-8")
                for window_name, start, end in (("is", split.is_start, split.is_end), ("oos", split.oos_start, split.oos_end)):
                    future = executor.submit(
                        _run_optimizer_window_candidate,
                        root=root,
                        args=args,
            ...
            for future in as_completed(futures):
                ...
                if "is" in bucket and "oos" in bucket:
                    ...
                    combined = {
                        ...
                        "is_returncode": bucket["is"].get("returncode"),
                        "oos_returncode": bucket["oos"].get("returncode"),
                        "elapsed_seconds": float(bucket["is"].get("elapsed_seconds", 0.0)) + float(bucket["oos"].get("elapsed_seconds", 0.0)),
                        "is_summary": is_summary,
                        "oos_summary": oos_summary,
                        "summary": summary,
                    }
        ...
        _write_csv(capital_dir / "stage_b_gate_risk_is_oos.csv", fieldnames=list(stage_b_csv_rows[0].keys()) if stage_b_csv_rows else ["rank"], rows=stage_b_csv_rows)
        ...
        best_summary = dict(best_record.get("summary", {})) if isinstance(best_record, dict) and isinstance(best_record.get("summary", {}), dict) else {}
    ...
    capital_results.sort(key=lambda item: (str(item.get("capital_currency", "")), float(item.get("capital_equity", 0.0))))
